...
// Start server and return the services to register with simrpc
type FstartServer func(ends []*simrpc.ClientEnd, grp Tgid, srv int, persister *Persister) []IService
...
func newGroups(net *simrpc.Network) *Groups {
...
func makeSrvGrp(net *simrpc.Network, gid Tgid, n int, mks FstartServer) *ServerGrp {
	sg := &ServerGrp{
		net:       net,
		srvs:      make([]*Server, n),
		gid:       gid,
...
func (sg *ServerGrp) StartServer(i int) {
	...
	for _, svc := range srv.svcs {
		s := simrpc.MakeService(svc)
		simsrv.AddService(s)
	}
	sg.net.AddServer(ServerName(sg.gid, i), simsrv)
}
