...
test/sim-all:
	@go test -v -race ./raft/tests/
# run election tests (simulation)
test/sim-election:
	@go test -v -race ./raft/tests/ -run 3A

# run replication tests (simulation)
test/sim-replication:
	@go test -v -race ./raft/tests/ -run 3B

# run persistennce tests (simulation)
test/sim-persistennce:
	@go test -v -race ./raft/tests/ -run 3C

# run snapshots tests (simulation)
test/sim-snapshots:
	@go test -v -race ./raft/tests/ -run 3D
